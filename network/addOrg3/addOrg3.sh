...
function printHelp () {
  ...
  echo "    -c <channel name> - test network channel name (defaults to \"settlement\")"
  ...
  echo "	addOrg3.sh up"
  echo "	addOrg3.sh up -c settlement -s couchdb"
...
function generateOrg3Definition() {
  ...
  set -x
  configtxgen -printOrg BankBMSP > ../organizations/peerOrganizations/bankb.sworna.example.com/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/bankb.sworna.example.com/peers/peer0.bankb.sworna.example.com/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/bankb.sworna.example.com" ]; then
    generateOrg3
...
#default for delay
CLI_DELAY=3
# channel name defaults to "settlement"

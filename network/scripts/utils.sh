...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"settlement\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c settlement -s couchdb "
    println "   network.sh up createChannel -bft -c settlement -s couchdb "
